...
On `sm` and wider viewports, each list remains a table. The current/recent table adds a fixed-width first column for activity. Active rows show a green dot whose opacity pulses gently; inactive rows leave the cell empty, so all user IDs align. The dot uses Tailwind's `animate-pulse` instead of an expanding `animate-ping` ring. Because this is a small status indicator, it remains animated even when reduced-motion is requested.
