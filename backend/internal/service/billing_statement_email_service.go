...
func ValidateBillingStatementEmailConfig(raw string) error {
	...
	for _, schedule := range []struct {
		...
		{name: "daily_schedule", spec: cfg.DailySchedule},
		{name: "weekly_schedule", spec: cfg.WeeklySchedule},
		{name: "monthly_schedule", spec: cfg.MonthlySchedule},
	} {
		spec := strings.TrimSpace(schedule.spec)
		if spec == "" {
			return fmt.Errorf("%s must not be empty", schedule.name)
		}
		if _, err := billingStatementCronParser.Parse(spec); err != nil {
			return fmt.Errorf("invalid %s: %w", schedule.name, err)
		}
	}
	return nil
}
...
func (s *BillingStatementEmailService) runStatementKind(parent context.Context, d billingStatementDef, now time.Time) {
	...
	if err != nil {
		log.Printf("[BillingStatement] invalid cron spec=%q for kind=%s", spec, d.kind)
		return
	}
	...
	if ctx.Err() != nil {
		log.Printf("[BillingStatement] send interrupted for kind=%s; last_run not updated", d.kind)
		return
	}
...
func (s *BillingStatementEmailService) isEmailDeliveryConfigured(ctx context.Context) bool {
	...
	if _, err := s.emailService.GetSMTPConfig(ctx); err != nil {
		log.Printf("[BillingStatement] email delivery is not configured; skipping")
		return false
	}
	return true
}
...
func (s *BillingStatementEmailService) sendStatements(ctx context.Context, kind string, periodName string, now time.Time) {
	...
	for {
		...
		if err != nil {
			log.Printf("[BillingStatement] list users page=%d failed", page)
			return
		}
		...
		for i := range users {
			...
			if err := emailCtx.Err(); err != nil && ctx.Err() == nil {
				log.Printf("[BillingStatement] send timed out or canceled for kind=%s user=%d", kind, user.ID)
			}
			cancel()
		}
...
func (s *BillingStatementEmailService) sendStatementToUser(ctx context.Context, user *User, periodName string, start, end time.Time, loc *time.Location) {
	...
	if err := s.emailService.SendEmail(ctx, user.Email, subject, body); err != nil {
		log.Printf("[BillingStatement] send email for user=%d failed", user.ID)
		return
	}
	if deliveryKey != "" && s.settingRepo != nil {
		if err := s.settingRepo.Set(ctx, deliveryKey, time.Now().UTC().Format(time.RFC3339Nano)); err != nil {
			log.Printf("[BillingStatement] record fallback delivery for user=%d failed", user.ID)
		}
	}
}
...
func (s *BillingStatementEmailService) billingStatementDeliveryExists(ctx context.Context, key string) bool {
	...
	log.Printf("[BillingStatement] check fallback delivery key failed; skipping duplicate-prone send")
	return true
}
...
func (s *BillingStatementEmailService) sendStatementWithTemplate(ctx context.Context, user *User, stmt *BillingStatement) bool {
	...
	if shouldFallbackNotificationEmail(err) {
		log.Printf("[BillingStatement] template email failed for user=%d; falling back to built-in body", user.ID)
		return false
	}
	log.Printf("[BillingStatement] send template email for user=%d failed", user.ID)
	return true
}
...
func (s *BillingStatementEmailService) aggregateUserUsage(ctx context.Context, userID int64, start, end time.Time) []BillingStatementLine {
	lines, err := s.usageRepo.GetBillingStatementLines(ctx, userID, start, end)
	if err != nil {
		log.Printf("[BillingStatement] query usage for user=%d failed", userID)
		return nil
	}
	if len(lines) == 0 {
		return nil
	...
	s.hydrateBillingStatementGroupNames(ctx, lines)
	sort.SliceStable(lines, func(i, j int) bool {
		if lines[i].Model != lines[j].Model {
			return lines[i].Model < lines[j].Model
		}
		if lines[i].GroupName != lines[j].GroupName {
			return lines[i].GroupName < lines[j].GroupName
		}
		return lines[i].BillingMode < lines[j].BillingMode
	})
	return lines
}
...
func (s *BillingStatementEmailService) tryAcquireLeaderLock(ctx context.Context) (func(), bool) {
	...
	if err != nil {
		log.Printf("[BillingStatement] leader lock SetNX failed; skipping")
		return nil, false
	}
	if !ok {
		return nil, false
