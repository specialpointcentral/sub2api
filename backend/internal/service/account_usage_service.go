...
type UsageLogRepository interface {
	...
	GetBillingStatementLines(ctx context.Context, userID int64, startTime, endTime time.Time) ([]BillingStatementLine, error)
...
const (
	...
	openAIQuotaProbeClaimTimeout      = 500 * time.Millisecond
)
...
func supportsAnthropicPassiveUsage(account *Account) bool {
	return account != nil && (account.IsAnthropicOAuthOrSetupToken() || account.IsKiro())
}
...
func (s *AccountUsageService) shouldProbeOpenAICodexSnapshotWithContext(ctx context.Context, accountID int64, now, lastProbeAt time.Time, force ...bool) bool {
	...
	if s.concurrencyService != nil {
		...
		claimBase := context.Background()
		if ctx != nil {
			claimBase = context.WithoutCancel(ctx)
		}
		claimCtx, cancel := context.WithTimeout(claimBase, openAIQuotaProbeClaimTimeout)
		claimed, err := s.concurrencyService.TryClaimOpenAIQuotaProbe(claimCtx, accountID, interval, scheduleVersion, initialNotBefore)
		cancel()
		if err == nil {
			if state != nil {
				state.mu.Lock()
