...
func TestKiroBridgeFailoverErrorPreservesWrappedFailover(t *testing.T) {
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	want := &UpstreamFailoverError{StatusCode: http.StatusPaymentRequired}

	got, ok := kiroBridgeFailoverError(c, &Account{ID: 7, Platform: PlatformAnthropic, Type: AccountTypeKiro}, fmt.Errorf("open stream: %w", want))

	require.True(t, ok)
	require.Same(t, want, got)
	require.Equal(t, http.StatusOK, rec.Code, "failover must not write a terminal 502 response")
}

func TestKiroUsageToClaudeDoesNotEstimateUncachedInputForCacheOnlyUsage(t *testing.T) {
	usage := kiroUsageToClaude(kiropkg.Usage{CacheReadInputTokens: 9, OutputTokens: 2, TotalTokens: 11}, 100)
...
func TestExecuteKiroUpstreamCooldownStoreFailuresDoNotReplaceUpstreamResponse(t *testing.T) {
	...
	storeErr := errors.New("redis transport unavailable")
	...
	for _, tt := range []struct {
		...
		{
			name:       "reserve failure fails open",
			status:     http.StatusOK,
			body:       `{}`,
			store:      &stubKiroCooldownStore{reserveErr: storeErr},
			wantStatus: http.StatusOK,
		},
		{
			name:       "mark success failure keeps success",
			status:     http.StatusOK,
			body:       `{}`,
			store:      &stubKiroCooldownStore{successErr: storeErr},
			wantStatus: http.StatusOK,
		},
		{
			name:       "mark 429 failure keeps 429",
			status:     http.StatusTooManyRequests,
			body:       `{"message":"rate limited"}`,
			store:      &stubKiroCooldownStore{mark429Err: storeErr},
			wantStatus: http.StatusTooManyRequests,
		},
		{
			name:       "mark suspended failure keeps suspended response",
			status:     http.StatusForbidden,
			body:       `{"reason":"TEMPORARILY_SUSPENDED"}`,
			store:      &stubKiroCooldownStore{suspendedErr: storeErr},
			wantStatus: http.StatusForbidden,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			upstream := &queuedHTTPUpstream{responses: []*http.Response{newJSONResponse(tt.status, tt.body)}}
			svc := &GatewayService{
			...
			resp, _, executeErr := svc.executeKiroUpstream(
				context.Background(), account, payloadBytes,
				"claude-sonnet-4-6", "claude-sonnet-4-6", "test-token", nil,
			)
...
func TestExecuteKiroUpstreamReserveCancellationStillPropagates(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	svc := &GatewayService{
		kiroCooldownStore: &stubKiroCooldownStore{reserveErr: context.Canceled},
	}

	_, _, err := svc.executeKiroUpstream(ctx, &Account{ID: 42}, []byte(`{}`), "claude-sonnet-4-6", "claude-sonnet-4-6", "token", nil)

	require.ErrorIs(t, err, context.Canceled)
}

func TestDoKiroMCPJSONRequestMarkSuspendedFailureKeepsUpstreamResponse(t *testing.T) {
	storeErr := errors.New("redis transport unavailable")
	upstream := &queuedHTTPUpstream{responses: []*http.Response{
		newJSONResponse(http.StatusForbidden, `{"reason":"TEMPORARILY_SUSPENDED"}`),
	}}
	account := &Account{ID: 42, Platform: PlatformAnthropic, Type: AccountTypeKiro, Concurrency: 1}
	svc := &GatewayService{
		httpUpstream:        upstream,
		kiroCooldownStore:   &stubKiroCooldownStore{suspendedErr: storeErr},
		tlsFPProfileService: &TLSFingerprintProfileService{},
	}

	resp, _, err := svc.doKiroMCPJSONRequest(context.Background(), account, "https://example.test/mcp", []byte(`{}`), "token")
...
func TestHandleKiroHTTPErrorLegacyKiroAPIKeyInvalidModelDoesNotFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	account := &Account{
