...
func latestCodexStableRelease(releases []*GitHubRelease) codexStableRelease {
	...
	for _, release := range releases {
		...
		if best.version == "" || CompareVersions(version, best.version) > 0 {
			publishedAt, parseErr := time.Parse(time.RFC3339, strings.TrimSpace(release.PublishedAt))
			if parseErr != nil {
				// Fail-open keeps the release eligible (zero time means the
				// stagger delay never blocks it); log so a malformed upstream
				// timestamp is visible instead of silently weakening staggering.
				slog.Warn("openai_codex_version_sync_published_at_parse_failed",
					"tag", tag, "published_at", release.PublishedAt, "error", parseErr)
			}
