...
func stagedCodexFingerprintIDs(c *gin.Context, account *Account) *codexFingerprintIDs {
	if c == nil || !isOpenAICodexFingerprintAccount(account) {
		return nil
	}
...
func shouldResolveCodexFingerprint(account *Account) bool {
	return isOpenAICodexFingerprintAccount(account) &&
		(!account.IsOpenAIApiKey() || account.GetCodexFingerprintMode() != codexFingerprintOff)
}
...
func prepareCodexFingerprintExtraForCreate(platform, accountType string, extra map[string]any) map[string]any {
	...
	if platform != PlatformOpenAI || (accountType != AccountTypeOAuth && accountType != AccountTypeAPIKey) || !codexFingerprintModeRequiresSeed(codexFingerprintModeFromExtra(prepared)) {
		return prepared
	}
	if prepared == nil {
