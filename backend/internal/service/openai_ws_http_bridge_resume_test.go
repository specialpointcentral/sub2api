...
func TestProxyOpenAIWSHTTPBridgeTurnOffModePreservesClientMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	// A historical seed left over after disabling convergence must not reactivate
	// any client_metadata rewrite on the bridge path either.
	account := &Account{
