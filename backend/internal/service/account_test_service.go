...
// "responses_compact" probes Codex remote compact v2 on the normal /responses path.
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if mode == AccountTestModeResponsesCompact {
		return s.testOpenAIResponsesCompactConnection(c, account, testModelID)
	}
...
// testOpenAIResponsesCompactConnection probes Codex remote compact v2 through
// the normal /responses endpoint. This records a separate capability from the
// legacy /responses/compact probe so routing can later prefer the better shape
// without losing the old fallback path.
func (s *AccountTestService) testOpenAIResponsesCompactConnection(c *gin.Context, account *Account, testModelID string) error {
	ctx := c.Request.Context()
	...
	payload := createOpenAIResponsesCompactProbePayload(testModelID)
	probeSessionID := compactProbeSessionID(account.ID)
	payload["prompt_cache_key"] = probeSessionID
	payloadBytes, _ := json.Marshal(payload)

	s.sendEvent(c, TestEvent{Type: "test_start", Model: testModelID})
	s.sendEvent(c, TestEvent{Type: "status", Text: "正在通过 /v1/responses body-signal 测试 Compact"})

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, apiURL, bytes.NewReader(payloadBytes))
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create Responses compact probe request")
	}
	req = req.WithContext(WithHTTPUpstreamProfile(req.Context(), HTTPUpstreamProfileOpenAI))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "text/event-stream")
	...
	if err != nil {
		if s.accountRepo != nil {
			updates := buildOpenAIResponsesCompactProbeExtraUpdates(nil, nil, err, time.Now())
			_ = s.accountRepo.UpdateExtra(ctx, account.ID, updates)
			mergeAccountExtra(account, updates)
		}
		return s.sendErrorAndEnd(c, fmt.Sprintf("Request failed: %s", err.Error()))
	...
	if s.accountRepo != nil {
		updates := buildOpenAIResponsesCompactProbeExtraUpdates(resp, body, nil, time.Now())
		if codexUpdates, err := extractOpenAICodexProbeUpdates(resp); err == nil && len(codexUpdates) > 0 {
			updates = mergeExtraUpdates(updates, codexUpdates)
		}
		if len(updates) > 0 {
	...
	s.sendEvent(c, TestEvent{Type: "content", Text: "Responses compact probe succeeded"})
	s.sendEvent(c, TestEvent{Type: "test_complete", Success: true})
	return nil
}
