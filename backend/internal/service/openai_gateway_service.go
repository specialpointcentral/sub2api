...
type OpenAIGatewayService struct {
	...
	openaiAccountRuntimeBlockReadCache  sync.Map // key: int64(accountID), value: openAIAccountRuntimeBlockReadCacheEntry
	...
	openAIRequestPacingSample           func(time.Duration, time.Duration) time.Duration
	openAIRequestPacingSleep            func(context.Context, time.Duration) error
...
//
// 语义说明（站点刻意决策）：upstream 的 isolateOpenAIUpstreamSessionID 还会
// 混入上游账号命名空间，使 failover 换号后同一客户端 session 在上游呈现为
// 全新身份（防跨账号关联）。站点在 compat/messages 桥接路径刻意保留会话
// 连续性——换号不中断客户端长会话（见 openai_oauth_passthrough_test.go 的
// preserves_compat_behavior 编码），其代价正是 upstream 想消除的跨账号
// session 复现。rectifier 收敛账号（device/session/full）由 staged 快照获得
// 每账号身份，两条语义并存；setup-token 与 OAuth 在该回退路径待遇一致。
