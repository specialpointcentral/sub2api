...
//
// 场景：管理员在渠道定价把 gpt-5.6-luna 的输入价配置成不同于官方价的 $0.4/M。
...
const (
	channelPricingInputTokens = 1_000_000
	// 1M 输入 token 下，渠道价的期望费用（USD）
...
func recordUsageWithChannelPricing(t *testing.T, requestedModel string, subscriptionGroup bool, pricings []ChannelModelPricing) (*UsageLog, *BillingService) {
	t.Helper()
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Usage: OpenAIUsage{
				InputTokens:  channelPricingInputTokens,
				OutputTokens: 0,
			},
	...
	return usageRepo.lastLog, svc.billingService
}

// 基线：请求模型与渠道定价 key 完全一致 → 按渠道价计。
func TestChannelPricing_ExactModelMatch(t *testing.T) {
	log, _ := recordUsageWithChannelPricing(t, "gpt-5.6-luna", false, []ChannelModelPricing{
...
func TestChannelPricing_SuffixedModelUsesNormalizedChannelPricing(t *testing.T) {
	log, _ := recordUsageWithChannelPricing(t, "gpt-5.6-luna-high", false, []ChannelModelPricing{
		tokenPricingForModels([]string{"gpt-5.6-luna"}, channelPricingExpectedChannelCost),
	})
	require.InDelta(t, channelPricingExpectedChannelCost, log.InputCost, 1e-9,
		"suffixed request model should fall back to the normalized channel pricing; got %v",
		log.InputCost)
}
...
func TestChannelPricing_DateSuffixedModelUsesNormalizedChannelPricing(t *testing.T) {
	log, _ := recordUsageWithChannelPricing(t, "gpt-5.6-luna-2026-08-01", false, []ChannelModelPricing{
		tokenPricingForModels([]string{"gpt-5.6-luna"}, channelPricingExpectedChannelCost),
	})
	require.InDelta(t, channelPricingExpectedChannelCost, log.InputCost, 1e-9,
...
func TestChannelPricing_SuffixedModelSubscriptionGroup(t *testing.T) {
	log, _ := recordUsageWithChannelPricing(t, "gpt-5.6-luna-high", true, []ChannelModelPricing{
		tokenPricingForModels([]string{"gpt-5.6-luna"}, channelPricingExpectedChannelCost),
	})
	require.InDelta(t, channelPricingExpectedChannelCost, log.InputCost, 1e-9)
}
...
func TestChannelPricing_UnrelatedChannelModelNotMatched(t *testing.T) {
	log, billingService := recordUsageWithChannelPricing(t, "gpt-5.6-luna-high", false, []ChannelModelPricing{
		tokenPricingForModels([]string{"gpt-5.4"}, channelPricingUnrelatedCost),
	})
	officialCost, err := billingService.CalculateCostUnified(CostInput{
		Ctx:            context.Background(),
		Model:          "gpt-5.6-luna-high",
		Group:          &Group{Platform: PlatformOpenAI, LongContextPricingEnabled: false},
		Tokens:         UsageTokens{InputTokens: channelPricingInputTokens},
		RateMultiplier: 1,
		Resolver:       NewModelPricingResolver(nil, billingService),
	})
	require.NoError(t, err)
	require.NotEqual(t, channelPricingUnrelatedCost, officialCost.InputCost,
		"test setup requires the unrelated channel price to differ from official pricing")
	require.InDelta(t, officialCost.InputCost, log.InputCost, 1e-9,
