...
func (s *OpenAIGatewayService) isOpenAIOAuth429Storm() bool {
	...
	if s.openAIAccountRuntimeCache != nil {
		cacheCtx, cancel := openAIAccountRuntimeCacheContext()
		count, err := s.openAIAccountRuntimeCache.GetOpenAIOAuth429StormCount(cacheCtx)
		cancel()
		if err == nil {
			return count >= openAIOAuth429StormThreshold
		}
		slog.Debug("openai_oauth_429_storm_cache_get_failed", "error", err)
	}
	windowStart := s.openaiOAuth429WindowStartUnixNano.Load()
