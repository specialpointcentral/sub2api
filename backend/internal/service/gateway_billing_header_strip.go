...
func stripBillingHeaderFromParsed(account *Account, parsed *ParsedRequest) {
	if account == nil || parsed == nil || !account.ShouldStripBillingHeader() || parsed.Body.Len() == 0 {
		return
	}
	stripped, changed := stripBillingHeaderSystemText(parsed.Body.Bytes())
	if !changed {
		return
	}
	_ = parsed.ReplaceBody(stripped)
}
