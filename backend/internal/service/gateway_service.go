...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	// Store sub2apiUserID in context so tryAcquireAccountSlot can encode it into slot members
	if sub2apiUserID > 0 {
		ctx = WithSub2APIUserID(ctx, sub2apiUserID)
	}
...
func (s *GatewayService) isAccountAllowedForPlatform(account *Account, platform string, useMixed bool) bool {
	...
	if useMixed {
		if account.MatchesLookupPlatform(platform) || isKiroAllowedInAnthropicScheduling(account, platform) {
			return true
		}
		return account.Platform == PlatformAntigravity && account.IsMixedSchedulingEnabled()
	}
	return account.MatchesLookupPlatform(platform) || isKiroAllowedInAnthropicScheduling(account, platform)
}

func isKiroAllowedInAnthropicScheduling(account *Account, platform string) bool {
	return account != nil && platform == PlatformAnthropic && account.IsKiro()
}
			...
			if err == nil && accountID > 0 && containsInt64(routingAccountIDs, accountID) {
				if _, excluded := excludedIDs[accountID]; !excluded {
					...
					if err == nil {
						...
						if !clearSticky && s.isAccountInGroup(account, groupID) && account.MatchesLookupPlatform(platform) && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) && s.isAccountSchedulableForQuota(account) && s.isAccountSchedulableForWindowCost(ctx, account, true) && s.isAccountSchedulableForRPM(ctx, account, true) && !s.isStickyAccountUpstreamRestricted(ctx, groupID, account, requestedModel) {
							if s.debugModelRoutingEnabled() {
				...
				if _, excluded := excludedIDs[accountID]; !excluded {
					...
					if err == nil {
						...
						if !clearSticky && s.isAccountInGroup(account, groupID) && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) && s.isAccountSchedulableForQuota(account) && s.isAccountSchedulableForWindowCost(ctx, account, true) && s.isAccountSchedulableForRPM(ctx, account, true) {
							if account.MatchesLookupPlatform(nativePlatform) || (account.Platform == PlatformAntigravity && account.IsMixedSchedulingEnabled()) {
								if s.debugModelRoutingEnabled() {
...
func isPlatformFilteredForSelection(acc *Account, platform string, allowMixedScheduling bool) bool {
	...
	if acc.MatchesLookupPlatform(platform) || isKiroAllowedInAnthropicScheduling(acc, platform) {
		return false
	}
	if allowMixedScheduling {
...
func (s *GatewayService) GetAccessToken(ctx context.Context, account *Account) (string, string, error) {
	switch account.Type {
	case AccountTypeOAuth, AccountTypeSetupToken, AccountTypeKiro:
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if shouldMimicClaudeCode {
		...
		normalizeOpts := claudeOAuthNormalizeOptions{
			...
			preserveToolChoice:      account.IsKiro(),
		}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// 计算费用
	opts.IsKiroAccount = account != nil && account.IsKiro()
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	...
	if platform != "" {
		...
		for _, acc := range accounts {
			if acc.MatchesLookupPlatform(platform) {
				filtered = append(filtered, acc)
			}
		}
		accounts = filtered
