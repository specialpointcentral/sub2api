...
// recordUsageOpts carries request attributes needed only by specialized
// fallback pricing. Long-context pricing is catalog-driven and does not belong
// here.
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	opts := &recordUsageOpts{IsKiroAccount: account != nil && account.IsKiro()}
...
func shouldUseKiroConservativeBillingFallback(result *ForwardResult, opts *recordUsageOpts) bool {
	if result == nil {
		return false
	}
...
func (s *GatewayService) calculateTokenCost(
	...
	if err != nil {
		...
		if shouldUseKiroConservativeBillingFallback(result, opts) {
