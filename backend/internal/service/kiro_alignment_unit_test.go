...
func TestGatewayServiceForwardCountTokens_KiroReturnsNotFound(t *testing.T) {
	...
	require.JSONEq(t, `{"type":"error","error":{"type":"not_found_error","message":"count_tokens endpoint is not supported for Kiro"}}`, recorder.Body.String())
	require.Equal(t, body, parsed.Body.Bytes(), "unsupported Kiro requests must short-circuit before body rewriting")
}

func TestGetBaseURL_LegacyKiroAPIKeyWithoutBaseURLUsesDefaultAnthropicBaseURL(t *testing.T) {
	account := Account{
...
func TestGatewayServiceKiroStreamKeepaliveDefaultsTo25Seconds(t *testing.T) {
	...
	got := svc.streamKeepaliveIntervalForAccount(&Account{Platform: PlatformAnthropic, Type: AccountTypeKiro})
...
func TestGatewayServiceKiroStreamKeepaliveUsesKiroSpecificConfig(t *testing.T) {
	...
	require.Equal(t, 25*time.Second, svc.streamKeepaliveIntervalForAccount(&Account{Platform: PlatformAnthropic, Type: AccountTypeKiro}))
...
func TestGatewayServiceRecordUsage_NormalizesKiroBillingModel(t *testing.T) {
	...
	err = svc.RecordUsage(context.Background(), &RecordUsageInput{
		...
		User:    &User{ID: 601},
		Account: &Account{ID: 701, Platform: PlatformAnthropic, Type: AccountTypeKiro},
	})
...
func TestGatewayServiceRecordUsage_KiroUnknownPricingFallsBackToConservativeCost(t *testing.T) {
	...
	err = svc.RecordUsage(context.Background(), &RecordUsageInput{
		...
		User:    &User{ID: 701},
		Account: &Account{ID: 801, Platform: PlatformAnthropic, Type: AccountTypeKiro},
	})
