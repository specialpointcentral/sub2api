...
func TestStripBillingHeaderFromParsedHonorsAccountFlag(t *testing.T) {
	...
	require.Equal(t, string(body), string(parsed.Body.Bytes()))
	require.Equal(t, "x-anthropic-billing-header: cc_version=2.1.121; cch=36db8;\nKeep this system prompt.", gjson.GetBytes(parsed.Body.Bytes(), "system").String())
	...
	require.JSONEq(t, `{
		...
		"system":"Keep this system prompt.",
		"messages":[{"role":"user","content":"hi"}]
	}`, string(parsed.Body.Bytes()))
	require.Equal(t, "Keep this system prompt.", gjson.GetBytes(parsed.Body.Bytes(), "system").String())
}
