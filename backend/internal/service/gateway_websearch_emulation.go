...
// Anthropic accounts keep the existing policy: API keys may force an account-level
// override, while OAuth/setup-token accounts use the default channel switch. Kiro
// is persisted as anthropic/kiro but uses only the logical Kiro channel switch.
func (s *GatewayService) shouldEmulateWebSearch(ctx context.Context, account *Account, groupID *int64, body []byte) bool {
	...
	if account.IsKiro() {
		return s.isChannelWebSearchEmulationEnabled(ctx, groupID, PlatformKiro)
	}
