...
func newAnthropicOAuthAccount() *Account {
	return &Account{
		ID:       3,
		Platform: PlatformAnthropic,
...
func TestShouldEmulateWebSearch_AnthropicOAuthFollowsChannel(t *testing.T) {
	mgr := websearch.NewManager([]websearch.ProviderConfig{{Type: "brave", APIKey: "k"}}, nil)
	SetWebSearchManager(mgr)
	defer SetWebSearchManager(nil)
	...
	channelSvc := newChannelServiceWithCache(79, ch)
	svc := &GatewayService{settingService: settingSvc, channelService: channelSvc}
	groupID := int64(79)

	require.True(t, svc.shouldEmulateWebSearch(context.Background(), newAnthropicOAuthAccount(), &groupID, webSearchToolBody))
}
