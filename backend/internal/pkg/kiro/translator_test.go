...
func TestParseNonStreamingEventStreamEstimatesOutputTokens(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{
			"content": "hello from kiro",
	...
	require.True(t, ok)
	require.Equal(t, float64(result.Usage.OutputTokens), usage["output_tokens"])
}

func TestParseNonStreamingEventStreamKeepsUpstreamOutputTokens(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{
			"content": "this text would estimate to a different value",
		},
	}))
	_, _ = stream.Write(buildEventStreamFrame(t, "messageMetadataEvent", map[string]any{
		"messageMetadataEvent": map[string]any{
...
func TestStreamEventStreamAsAnthropicEstimatesOutputTokens(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{
			"content": "Hello from Kiro",
	...
	output := out.String()
	require.Contains(t, output, `"output_tokens":`+fmt.Sprint(result.Usage.OutputTokens))
}

func TestStreamEventStreamAsAnthropicEstimatesOutputTokensAfterAccumulatingChunks(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	for i := 0; i < 8; i++ {
		_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
			"assistantResponseEvent": map[string]any{
	...
	require.Contains(t, out.String(), `"output_tokens":2`)
}

func TestStreamEventStreamAsAnthropicKeepsUpstreamOutputTokens(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{
			"content": "this text would estimate to a different value",
	...
	require.Contains(t, out.String(), `"output_tokens":7`)
}
