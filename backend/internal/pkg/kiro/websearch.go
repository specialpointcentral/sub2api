...
func InjectSearchIndicatorsInResponse(responsePayload []byte, searches []SearchIndicator) ([]byte, error) {
	...
	if len(searches) > math.MaxInt/2 {
		return responsePayload, fmt.Errorf("too many search indicators")
	}
	searchContentLen := len(searches) * 2
	if len(content) > math.MaxInt-searchContentLen {
		return responsePayload, fmt.Errorf("response content is too large")
	}
	updated := make([]any, 0, searchContentLen+len(content))
