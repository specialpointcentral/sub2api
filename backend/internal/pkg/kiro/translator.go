...
type outputTokenEstimator struct {
...
func (e *outputTokenEstimator) AddText(text string) {
	if e == nil || text == "" {
		return
	}
	_, _ = e.text.WriteString(text)
}

func (e *outputTokenEstimator) AddJSONValue(value any) {
	if e == nil || value == nil {
		return
	}
	...
	e.AddText(string(encoded))
}

func (e *outputTokenEstimator) AddJSONText(text string) {
	if e == nil || text == "" {
		return
	}
	e.AddText(text)
...
func (e *outputTokenEstimator) ApplyFallback(usage *Usage) {
	if e == nil || usage == nil || usage.OutputTokens > 0 || e.text.Len() == 0 {
		return
	}
	tokens := estimateTextTokens(e.text.String())
	if tokens <= 0 {
		return
	}
...
func MapModel(model string) string {
	...
	case "claude-opus-4-8", "claude-opus-4-8-thinking", "claude-opus-4.8", "claude-opus-4.8-thinking":
		return "claude-opus-4.8"
	case "claude-opus-4-7", "claude-opus-4-7-thinking", "claude-opus-4.7", "claude-opus-4.7-thinking":
	...
	case "claude-sonnet-4-5", "claude-sonnet-4-5-thinking", "claude-sonnet-4-5-20250929", "claude-sonnet-4-5-20250929-thinking", "claude-sonnet-4.5":
		return "claude-sonnet-4.5"
	case "claude-haiku-4-5", "claude-haiku-4-5-thinking", "claude-haiku-4-5-20251001", "claude-haiku-4-5-20251001-thinking", "claude-haiku-4.5":
...
func StreamEventStreamAsAnthropicWithContext(ctx context.Context, body io.Reader, w io.Writer, model string, inputTokens int, requestCtx KiroRequestContext) (*StreamResult, error) {
	...
	var outputEstimator outputTokenEstimator
	contentBlockIndex := -1
	...
	emitStreamingToolInput := func(toolUseID, name, fragment string) error {
		...
		outputEstimator.AddJSONText(fragment)
	...
	emitToolUse := func(tool KiroToolUse) error {
		...
		outputEstimator.AddJSONText(string(inputJSON))
	...
	outputEstimator.ApplyFallback(&usage)
	if usage.TotalTokens == 0 {
		usage.TotalTokens = usage.InputTokens + usage.OutputTokens
	}
	if stopReason == "" {
...
func parseEventStream(body io.Reader) (string, []KiroToolUse, Usage, string, error) {
	...
	if usage.OutputTokens == 0 {
		var outputEstimator outputTokenEstimator
		for _, block := range extractThinkingBlocks(cleanText) {
			switch block["type"] {
			case "thinking":
		...
		for _, tool := range toolUses {
			...
			outputEstimator.AddJSONValue(tool.Input)
		}
		outputEstimator.ApplyFallback(&usage)
	}
	if usage.TotalTokens == 0 {
