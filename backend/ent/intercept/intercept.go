...
// The ModelRateLimitRuleFunc type is an adapter to allow the use of ordinary function as a Querier.
type ModelRateLimitRuleFunc func(context.Context, *ent.ModelRateLimitRuleQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ModelRateLimitRuleFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ModelRateLimitRuleQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ModelRateLimitRuleQuery", q)
}

// The TraverseModelRateLimitRule type is an adapter to allow the use of ordinary function as Traverser.
type TraverseModelRateLimitRule func(context.Context, *ent.ModelRateLimitRuleQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseModelRateLimitRule) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseModelRateLimitRule) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ModelRateLimitRuleQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ModelRateLimitRuleQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ModelRateLimitRuleQuery:
		return &query[*ent.ModelRateLimitRuleQuery, predicate.ModelRateLimitRule, modelratelimitrule.OrderOption]{typ: ent.TypeModelRateLimitRule, tq: q}, nil
