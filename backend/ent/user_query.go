...
type UserQuery struct {
	...
	withModelRateLimitRules   *ModelRateLimitRuleQuery
...
// QueryModelRateLimitRules chains the current query on the "model_rate_limit_rules" edge.
func (_q *UserQuery) QueryModelRateLimitRules() *ModelRateLimitRuleQuery {
	query := (&ModelRateLimitRuleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withModelRateLimitRules:   _q.withModelRateLimitRules.Clone(),
...
// WithModelRateLimitRules tells the query-builder to eager-load the nodes that are connected to
// the "model_rate_limit_rules" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithModelRateLimitRules(opts ...func(*ModelRateLimitRuleQuery)) *UserQuery {
	query := (&ModelRateLimitRuleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withModelRateLimitRules = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [15]bool{
			...
			_q.withModelRateLimitRules != nil,
	...
	if query := _q.withModelRateLimitRules; query != nil {
		if err := _q.loadModelRateLimitRules(ctx, query, nodes,
			func(n *User) { n.Edges.ModelRateLimitRules = []*ModelRateLimitRule{} },
			func(n *User, e *ModelRateLimitRule) {
				n.Edges.ModelRateLimitRules = append(n.Edges.ModelRateLimitRules, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadModelRateLimitRules(ctx context.Context, query *ModelRateLimitRuleQuery, nodes []*User, init func(*User), assign func(*User, *ModelRateLimitRule)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(modelratelimitrule.FieldUserID)
	}
	query.Where(predicate.ModelRateLimitRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ModelRateLimitRulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
