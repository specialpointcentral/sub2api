export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexDevicePoolSizeError: '设备池大小只能是 1，或 3–8 的整数。',
        codexDevicePoolPlatformRatio: '平台配额比例（Mac : Windows : Linux）',
        codexDevicePoolPlatformRatioHint: '填写三个 1–1,000,000 的整数，例如 1:1:2。每个平台至少保留 1 个设备坑位；设备按真实平台懒创建，比例或池大小扩容会影响后续 rendezvous 分配。',
        codexDevicePoolPlatformRatioError: '平台配额比例必须是 mac:windows:linux 顺序的三个 1–1,000,000 整数，例如 1:1:2。',
        codexUAPersona: '启用 UA persona',
        codexUAPersonaHint: '默认关闭。开启后按稳定设备身份派生开发者工具用户分布的 Mac / Ubuntu / Windows Codex 指纹，并保持 sandbox 与 OS 一致。',
        codexVersionStaggerMaxHours: '版本采用最大错峰（小时）',
        codexVersionStaggerMaxHoursHint: '范围 0–48。每个部署使用 JWT secret 派生稳定延迟；0 表示同步到正式版后立即采用。',
        openAIRequestPacing: 'OpenAI 账号（OAuth 与 API Key）请求起步整形',
        openAIRequestPacingHint: '默认关闭。开启后，同一 OpenAI 账号（OAuth 与 API Key）的推理 turn 按均匀随机间隔起步；排队请求继续等待，不另建并发系统。',
        openAIRequestPacingMinInterval: '最小起步间隔（毫秒）',
        openAIRequestPacingMaxInterval: '最大起步间隔（毫秒）',
        openAIRequestPacingRangeError: 'OpenAI 请求间隔必须为 0–60000 毫秒的整数，且最小值不能大于最大值。',
        openAIAccountThreadConcurrencyLimit: '单 OAuth 账号活跃 thread 上限',
        openAIAccountThreadConcurrencyLimitHint: '0 表示不限并保持现状；正数通过现有账号并发槽位限制同时在飞的推理 turn。',
        openAIAccountThreadConcurrencyLimitError: '活跃 thread 上限必须为 0–10000 的整数。',
        openAIQuotaProbeInterval: 'Quota 探针间隔（分钟）',
        openAIQuotaProbeJitterRatio: 'Quota 探针抖动比例',
        openAIQuotaProbeJitterRatioHint: '0.25 表示每次自动探针均匀分布在基础间隔的 75%–125%；管理员手动刷新不受影响。',
        openAIQuotaProbeRangeError: 'Quota 探针间隔必须为 1–1440 分钟，抖动比例必须为 0–0.5。',
