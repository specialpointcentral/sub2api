export default {
    accounts: {
      ...
      types: {
        ...
        kiroOauth: 'OAuth / AWS Builder ID / 导入',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '适用于 OpenAI OAuth 与 API Key 账号。OAuth 的关闭模式保留账号级 session 命名空间；API Key 的关闭模式不启用新增的指纹收敛，并保留各入口既有的 session 兼容行为，即使保留历史 seed。仅设备 / 设备+会话 / 完全收敛均为显式启用；仅上游为第三方中转时建议开启。部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
        codexFingerprintOff: '关闭（API Key 不启用新增指纹收敛）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        codexFingerprintFull: '完全收敛',
        nonCodexTrafficPolicy: '非 Codex 流量',
        nonCodexTrafficPolicyDesc: '控制本 OpenAI 账号的非官方 Codex 客户端流量。直连 OpenAI 上游时保持关闭；上游为第三方中转的 API Key 账号也可开启 pi 归一化。WebSocket 流量仍保持透传。',
        nonCodexTrafficOff: '关闭（透传）',
        nonCodexTrafficPi: '使用 pi 归一化',
      ...
      last30DaysUsage: '使用统计（日均基于实际使用天数）',
      stats: {
        dateRange: '时间范围',
        dateRangeToday: '今天',
        dateRangeYesterday: '昨天',
        dateRange7d: '7 天',
        dateRange30d: '30 天',
        dateRangeMonth: '本月',
        dateRange90d: '90 天',
        ...
        todayCost: '今日费用',
        currentUsers: '当前/最近用户',
        currentUsersHint: '近 5 分钟，含正在处理的请求',
        rangeUsers: '范围内用户',
        last30Days: '30 天',
        user: '用户',
        email: '邮箱',
        ...
        lastUsedAt: '最后使用',
        noRecentUsers: '暂无当前或最近用户',
        noRangeUsers: '该时间范围内暂无用户使用记录',
        usageTrend: '费用与请求趋势',
        noData: '该账号暂无使用数据'
      }
    },
