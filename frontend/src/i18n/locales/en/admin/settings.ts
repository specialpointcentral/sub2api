export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexDevicePoolSizeError: 'Device pool size must be 1 or an integer from 3 to 8.',
        codexDevicePoolPlatformRatio: 'Platform quota ratio (Mac : Windows : Linux)',
        codexDevicePoolPlatformRatioHint: 'Enter three integers from 1 to 1,000,000, such as 1:1:2. Every platform keeps at least one slot; devices are created lazily from observed platforms, and expanding the ratio or pool size affects subsequent rendezvous assignment.',
        codexDevicePoolPlatformRatioError: 'Platform quota ratio must contain three integers from 1 to 1,000,000 in mac:windows:linux order, such as 1:1:2.',
        codexUAPersona: 'Enable UA personas',
        codexUAPersonaHint: 'Off by default. When enabled, each stable device gets a Mac, Ubuntu, or Windows Codex fingerprint following developer-tool user distribution, with sandbox aligned to the OS.',
        codexVersionStaggerMaxHours: 'Maximum version adoption stagger (hours)',
        codexVersionStaggerMaxHoursHint: 'Range 0–48. Each deployment derives a stable delay from its JWT secret; 0 adopts a stable release immediately after sync.',
        openAIRequestPacing: 'Pace OpenAI accounts (OAuth and API Key)',
        openAIRequestPacingHint: 'Disabled by default. When enabled, inference turns on OpenAI accounts (OAuth and API Key) start at uniformly random intervals; queued requests wait without using a second concurrency system.',
        openAIRequestPacingMinInterval: 'Minimum start interval (ms)',
        openAIRequestPacingMaxInterval: 'Maximum start interval (ms)',
        openAIRequestPacingRangeError: 'OpenAI request pacing intervals must be integers from 0 to 60000 ms, with minimum no greater than maximum.',
        openAIAccountThreadConcurrencyLimit: 'Active threads per OAuth account',
        openAIAccountThreadConcurrencyLimitHint: '0 keeps the current unlimited behavior. A positive value caps active inference turns through the existing account concurrency slots.',
        openAIAccountThreadConcurrencyLimitError: 'The active-thread limit must be an integer from 0 to 10000.',
        openAIQuotaProbeInterval: 'Quota probe interval (minutes)',
        openAIQuotaProbeJitterRatio: 'Quota probe jitter ratio',
        openAIQuotaProbeJitterRatioHint: '0.25 schedules each automatic probe uniformly between 75% and 125% of the base interval. Manual refresh is unaffected.',
        openAIQuotaProbeRangeError: 'Quota probe interval must be 1–1440 minutes and jitter ratio must be 0–0.5.',
