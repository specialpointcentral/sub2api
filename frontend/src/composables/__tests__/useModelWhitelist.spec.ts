...
describe('useModelWhitelist', () => {
  ...
  it('kiro 预设映射只暴露 Claude 入口', () => {
    ...
    expect(mappings.map(({ from, to }) => ({ from, to }))).toEqual([
      ...
      { from: 'claude-haiku-4-5', to: 'claude-haiku-4.5' },
      { from: 'claude-haiku-4-5-thinking', to: 'claude-haiku-4.5' },
  ...
  it('kiro 白名单保存时会转换为 Kiro 上游模型映射', () => {
    const mapping = buildKiroModelMappingObject('whitelist', ['claude-opus-4-8', 'claude-opus-4-8-thinking', 'claude-opus-4-7'], [])
