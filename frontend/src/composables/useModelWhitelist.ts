...
const kiroPresetMappings = [
  { label: 'Opus 4.8', from: 'claude-opus-4-8', to: 'claude-opus-4.8', color: 'bg-fuchsia-100 text-fuchsia-700 hover:bg-fuchsia-200 dark:bg-fuchsia-900/30 dark:text-fuchsia-300' },
  { label: 'Opus 4.8 Thinking', from: 'claude-opus-4-8-thinking', to: 'claude-opus-4.8', color: 'bg-fuchsia-100 text-fuchsia-700 hover:bg-fuchsia-200 dark:bg-fuchsia-900/30 dark:text-fuchsia-300' },
  ...
  { label: 'Haiku 4.5', from: 'claude-haiku-4-5', to: 'claude-haiku-4.5', color: 'bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-300' },
  { label: 'Haiku 4.5 Thinking', from: 'claude-haiku-4-5-thinking', to: 'claude-haiku-4.5', color: 'bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-300' },
...
const kiroDefaultMappingByFrom = new Map(kiroDefaultMappings.map(({ from, to }) => [from, to]))
...
export function isKiroDefaultModelMapping(from: string, to: string): boolean {
  return kiroDefaultMappingByFrom.get(from.trim()) === to.trim()
}

export function buildKiroModelMappingObject(
  mode: ModelRestrictionMode,
  allowedModels: string[],
  modelMappings: ModelMappingEntry[]
): Record<string, string> | null {
  if (mode !== 'whitelist') {
    return buildModelMappingObject(mode, allowedModels, modelMappings)
  }

  const mapping: Record<string, string> = {}
  for (const model of allowedModels) {
    const normalizedModel = model.trim()
    if (!normalizedModel || normalizedModel.includes('*')) continue
    mapping[normalizedModel] = kiroDefaultMappingByFrom.get(normalizedModel) ?? normalizedModel
  }
