...
describe('AccountStatsModal responsive user lists', () => {
  ...
  it('renders activity in a dedicated desktop column and keeps inactive rows aligned', async () => {
    ...
    const desktopActiveIndicator = activeCells[0].get('.current-user-active-indicator')
    expect(desktopActiveIndicator.classes()).toContain('animate-pulse')
    expect(desktopActiveIndicator.classes()).not.toContain('motion-reduce:animate-none')
    expect(desktopActiveIndicator.find('.animate-ping').exists()).toBe(false)
  ...
  it('renders complete mobile rows without a horizontally scrolling table', async () => {
    ...
    const mobileActiveIndicator = currentMobile.get('.current-user-active-indicator')
    expect(mobileActiveIndicator.classes()).toContain('animate-pulse')
    expect(mobileActiveIndicator.classes()).not.toContain('motion-reduce:animate-none')
    expect(mobileActiveIndicator.find('.animate-ping').exists()).toBe(false)
