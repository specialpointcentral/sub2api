...
describe('ModelRateLimitRuleEditor', () => {
  it('opens and filters model candidates while preserving free-text entry', async () => {
    const wrapper = mount(ModelRateLimitRuleEditor, {
      attachTo: document.body,
      props: {
        modelValue: [],
        candidates: ['claude-opus-4-1', 'gpt-5.6-luna-high', 'gpt-5.6-sol'],
      },
    })

    await wrapper.get('[data-test="add-rule"]').trigger('click')
    const pattern = wrapper.get('[data-test="model-pattern"]')
    await pattern.trigger('focus')

    expect(wrapper.get('[data-test="model-candidate-dropdown"]').classes()).toContain('select-dropdown')
    expect(wrapper.findAll('[data-test="model-candidate"]').map((option) => option.text())).toEqual([
      'claude-opus-4-1',
      'gpt-5.6-luna-high',
      'gpt-5.6-sol',
    ])

    await pattern.setValue('luna')
    expect(wrapper.findAll('[data-test="model-candidate"]').map((option) => option.text())).toEqual([
      'gpt-5.6-luna-high',
    ])

    await wrapper.get('[data-test="model-candidate"]').trigger('click')
    expect((pattern.element as HTMLInputElement).value).toBe('gpt-5.6-luna-high')
    expect(pattern.attributes('aria-expanded')).toBe('false')
    expect(wrapper.emitted('update:modelValue')?.at(-1)?.[0]).toEqual([
      { model_pattern: 'gpt-5.6-luna-high', limits: { concurrency: 0, rpm: 0 } },
    ])

    await pattern.setValue('gpt-5.6-luna*')
    expect((pattern.element as HTMLInputElement).value).toBe('gpt-5.6-luna*')
    expect(wrapper.emitted('update:modelValue')?.at(-1)?.[0]).toEqual([
      { model_pattern: 'gpt-5.6-luna*', limits: { concurrency: 0, rpm: 0 } },
    ])
