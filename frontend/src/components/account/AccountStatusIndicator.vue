...
const isKiroAccount = computed(() => props.account.platform === 'anthropic' && props.account.type === 'kiro')

// Computed: is rate limited (429)
const activeKiroRuntimeResetAt = computed(() => {
  if (!isKiroAccount.value) return null
...
const isKiroRuntimeSuspended = computed(() => {
  if (!isKiroAccount.value) return false
