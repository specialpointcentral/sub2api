<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="((account.platform === 'openai' || account.platform === 'grok') && account.type === 'oauth') || isKiroOAuthAccount"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <label class="input-label">{{ t('admin.accounts.modelRestriction') }}</label>
        ...
        <template v-else-if="isKiroOAuthAccount">
          <div class="mb-4 flex gap-2">
            <button
              type="button"
              @click="modelRestrictionMode = 'whitelist'"
...
const isKiroOAuthAccount = computed(() => props.account?.platform === 'anthropic' && props.account?.type === 'kiro')
...
const applyKiroModelMappings = (entries: Array<[string, string]>) => {
  const isWhitelistMode = entries.length > 0 && entries.every(([from, to]) => from === to || isKiroDefaultModelMapping(from, to))
  if (isWhitelistMode) {
...
const loadDefaultKiroModelMappings = () => {
  fetchKiroDefaultMappings().then(mappings => {
    ...
    modelRestrictionMode.value = 'whitelist'
    allowedModels.value = mappings.map(({ from }) => from)
    modelMappings.value = []
  })
}
...
const presetMappings = computed(() => getPresetMappingsByPlatform(
  isKiroOAuthAccount.value ? 'kiro' : (props.account?.platform || 'anthropic')
))
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (isKiroOAuthAccount.value && newAccount.credentials) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isKiroOAuthAccount.value) {
      const currentCredentials = (updatePayload.credentials as Record<string, unknown>) ||
        ((props.account.credentials as Record<string, unknown>) || {})
      const newCredentials: Record<string, unknown> = { ...currentCredentials }

      const modelMapping = buildKiroModelMappingObject(modelRestrictionMode.value, allowedModels.value, modelMappings.value)
      if (modelMapping) {
        newCredentials.model_mapping = modelMapping
      } else {
        delete newCredentials.model_mapping
