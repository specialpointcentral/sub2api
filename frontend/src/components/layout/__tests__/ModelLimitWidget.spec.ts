...
describe('ModelLimitWidget', () => {
  ...
  it('keeps overall concurrency as default, switches only on saturation, renders complete accessible groups, and honors unlimited and color boundaries', async () => {
    ...
    const unsaturatedRows = detailCard.findAll('[data-test="usage-progress"]')
    expect(unsaturatedRows).toHaveLength(4)
    for (const row of unsaturatedRows) {
      const progressRow = row.get('[data-test="usage-progress-row"]')
      const warningSlot = progressRow.get('[data-test="saturation-warning-slot"]')
      expect(progressRow.element.firstElementChild).toBe(warningSlot.element)
      expect(warningSlot.classes()).toEqual(expect.arrayContaining(['h-3.5', 'w-3.5', 'shrink-0']))
      expect(warningSlot.find('[data-test="saturation-warning"]').exists()).toBe(false)
    }
    ...
    for (const row of saturatedRows) {
      const progressRow = row.get('[data-test="usage-progress-row"]')
      const warningSlot = progressRow.get('[data-test="saturation-warning-slot"]')
      const warning = warningSlot.get('[data-test="saturation-warning"]')
      expect(progressRow.element.firstElementChild).toBe(warningSlot.element)
